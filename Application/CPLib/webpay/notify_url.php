...
canpointCommon::logDegub("notify_alipay_config=" . json_encode($alipay_config), ".web");
//计算得出通知验证结果
$alipayNotify = new AlipayNotify($alipay_config);
canpointCommon::logDegub("notify_alipayNotify=" . json_encode($alipayNotify), ".web");
$verify_result = $alipayNotify->verifyNotify();
canpointCommon::logDegub("notify_verify_result=" . $verify_result, ".web");
if ($verify_result) {//验证成功
    ...
    if ($_POST['trade_status'] == 'TRADE_FINISHED' || $_POST['trade_status'] == 'TRADE_SUCCESS') {
        ...
        if ($is_pay['payorno'] == 1) {
            ...
            canpointCommon::logDegub("notify_new_rpoint=" . $mony['u_rpoint'], '.web');
            if ($study->where("u_guid='" . $is_pay['memberid'] . "'")->save($mony)) {
                canpointCommon::logDegub("notify_user_sql=" . $study->getLastSql(), '.web');
                //修改红包状态
        ...
        canpointCommon::logError("notify_mesg_Paid trade_status=" . $_GET['trade_status']);
    }
...
//    }
    ...
    //验证失败
    //echo "fail";
    canpointCommon::logError("notify_recharge_mesg=Verify failure");
    //调试用，写文本函数记录程序运行情况是否正常
    //logResult("这里写入想要调试的代码变量值，或其他运行的结果记录");
}
?>