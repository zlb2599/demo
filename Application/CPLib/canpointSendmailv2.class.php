...
#protected $SMTPServer = "mail.canpoint.net"; // SMTP 서버 도메인 
protected $SMTPServer = "10.162.57.149"; // SMTP 서버 도메인 
protected $SMTPPort = 25; // Port 
protected $SMTPAuthUser="noreply@mail.canpoint.net"; // SMTP 인증 사용자 
protected $SMTPAuthPasswd="Haifengyhn654852"; // SMTP 인증 비밀번호 
...
if($this->Socket) 
{ 
	$s = $this->_sockPut('EHLO ' . $this->SMTPServer); 
	if($this->SMTPAuthUser) 
	{ // SMTP 인증 
		$s = $this->_sockPut('AUTH LOGIN'); 
		$s = $this->_sockPut( trim( base64_encode($this->SMTPAuthUser) ) ); 
		$s = $this->_sockPut( trim( base64_encode($this->SMTPAuthPasswd) )); 
		...
		if(strpos($Result, '250') !== false) $Succ++; // 성공여부판단
...
{
	//$Succ = $this->_localSend(); // 외부 SMTP 서버를 이용하지 않으면 로컬 SMTP를 이용해서 전송한다 
}
