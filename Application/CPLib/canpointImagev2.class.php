...
class canpointImagev2 {
    ...
    function showImg($imgFile, $text = '', $x = '10', $y = '10', $alpha = '50') {
        ...
        $info = canpointImagev2::getImageInfo($imgFile);
        if ($info !== false) {
            ...
            if ($im) {
                ...
                if (!empty($text)) {
                    ...
                    if (is_file($text) && file_exists($text)) {//判断$text是否是图片路径
                        // 取得水印信息
                        $textInfo = canpointImagev2::getImageInfo($text);
    ...
    static function thumb($image, $thumbname, $type = '', $maxWidth = 200, $maxHeight = 50, $interlace = true) {
        // 获取原图信息
        $info = canpointImagev2::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
    ...
    static function buildString($string, $rgb = array(), $filename = '', $type = 'png', $disturb = 1, $border = true) {
        ...
        canpointImagev2::output($im, $type, $filename);
    }
    ...
    {
    	//import('@.ORG.String');
    	$randval = canpointString::randString($length, $mode);
        $_SESSION[$verifyName] = md5($randval);
    	canpointCommon::logDegub( 'user sessionid=' . session_id() . ', ' . 'loginmode=' . $verifyName . ', ' . 'code=' . $randval . ', serverCode_md5_in_session=' . $_SESSION[$verifyName], '.g1' );
        ...
        canpointImagev2::output($im, $type);
    }
