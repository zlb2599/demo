...
function parse_name($name, $type=0) {
    if ($type) {
        return ucfirst(preg_replace_callback('/_([a-zA-Z])/', function($match){return strtoupper($match[1]);}, $name));//haifeng edit 2014.11.04
 ...
 * @return Model
 * haifeng 2015-02-05 修改
 */
function D($name='',$layer='', $other_dir_mode='') 
{
    ...
    }elseif(false === strpos($name,'/'))
    {
        ...
        {
        	if( !empty( $other_dir_mode ) )
        	{
        		import( $other_dir_mode . '/'.$layer.'/'.$class);
        	}
        	else
        	{
