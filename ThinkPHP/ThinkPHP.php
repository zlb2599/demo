...
if(!IS_CLI) {
    ...
    if(!defined('_PHP_FILE_')) {
        if(IS_CGI) {
            ...
            //define('_PHP_FILE_',    rtrim(str_replace($_SERVER['HTTP_HOST'],'',$_temp[0].'.php'),'/'));//haifeng 2014.11.18
            //define('_PHP_FILE_',    rtrim(str_replace($_SERVER['HTTP_HOST'],'',$_temp[0].''),'/'));
            define('_PHP_FILE_',   '');//haifeng 2014.11.18 在nginx 下，这个定义是对的
        }else {
    ...
    'appenders' => array(
        'default' => array(
            ...
            'layout' => array(
    			...
    			'params' => array(
                    'conversionPattern' => '%date{Y-m-d H:i:s:u} %-12server{REMOTE_ADDR} %-4server{REMOTE_PORT} %relative %-5le %class %method:%line DID:%ses{CP_LOGIN_USER_DID} UID:%ses{CP_LOGIN_USER_ID} | %msg%n'
                )
            ),
            'params' => array(
            	'file' => '/alidata1/ApiLog/pay.canpoint-%s.log',
            	'datePattern'=>'Y-m-d',
            	'append' => true
            )
        )
